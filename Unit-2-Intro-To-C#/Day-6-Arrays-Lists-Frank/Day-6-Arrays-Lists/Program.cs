...
class Program
{
    static void Main(string[] args)
    {
        ...
        int[] ints = new int[3];  // numbers.Length=5; 4 is the largest allowable index
        ...
        for (int i=0; i < ints.Count;      i++)  // i = 0, 1, 2 inside loop and 3 when exit loop
        {
            Console.WriteLine("Please enter a number:  "); // Asking for the number
            ints[i] = int.Parse(Console.ReadLine());    // get a string from keyboard
                                                           // convert it toan int
        ...
        for (int i=0; i < ints.Count; i++) // i=0, 1, 2 
        {
            Console.WriteLine("Element #"+ i + " is: " + ints[i]);
            sum = sum + ints[i]; // add the current number in the array to sum
        }
        ...
        Console.WriteLine("The average of the numbers is: " +  (double) sum / (double) ints.Length);
